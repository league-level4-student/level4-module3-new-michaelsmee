...
public class SpiesOnATrain {
	...
	String findIntel(LinkedList<TrainCar> train, String[] clues) {
		...
		HashMap<String, Integer> cluesList = new HashMap<String, Integer>();
		...
		for (int i = 0; i < train.size() - 1; i++) {
			...
			String[] list = evidence.split(" ");
			// String suspectName = list[0];
			String suspectName = list[0];

			for (int x = 0; x < clues.length; x++) {

				// String Q = current.getValue().questionPassenger().toString();
				if (current.getValue().questionPassenger().contains(clues[x])) {
					leads.add(current.getValue().toString());
					//String spy = evidence.substring(startName, evidence.indexOf(' '));
					if(cluesList.containsKey(suspectName)) {
					cluesList.put(suspectName, cluesList.get(suspectName) + 1);
					}
					else {
					cluesList.put(suspectName, 1);
					}
				}
		...
		for(String key: cluesList.keySet()) {
			if(cluesList.get(key) == 3) {
				return key;
			}
		}
